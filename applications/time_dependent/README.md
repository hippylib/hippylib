...
\sum_{n=1}^{N} r(u_n, u_{n-1}, m, p_n) = 0 \quad \forall p_n
$$
...
$$
\partial_u q(u_k, m) \tilde{u} + \partial_1 r(u_k, u_{k-1}, m, p_k) \tilde{u}
+ \partial_2 r(u_{k+1}, u_k, m, p_{k+1}) \tilde{u} 
= 0,
$$
...
\; \underbrace{- \partial_u q(u_k, m) \tilde{u}}_{\text{ given by adj\_rhs}}
$$
...
\begin{align}
L^H &= 
\sum_{n=1}^{N} r(u_n, u_{n-1}, m, \hat{p}_n) \\
& + 
\sum_{n=1}^{N} \partial_u q(u_n, m) \hat{u}_n + \partial_1 r(u_n, u_{n-1}, m, p_n) \hat{u}_n
+ \partial_2 r(u_{n+1}, u_n, m, p_{n+1}) \hat{u}_n  \\
& + \sum_{n=1}^{N} \partial_m r(u_n, u_{n-1}, m, p_n) \hat{m}
\end{align}
$$
...
\\
& + 
\partial_u^2 q(u_k, m) \hat{u}_k \tilde{u}_k \\
& + \partial_1 \partial_1 r(u_k, u_{k-1}, m, p_k) \hat{u}_k \tilde{u}_k \\
& + \partial_2 \partial_1 r(u_{k}, u_{k-1}, m, p_{k}) \hat{u}_{k-1} \tilde{u}_k \\
& + \partial_2 \partial_2 r(u_{k+1}, u_k, m, p_{k+1}) \hat{u}_k \tilde{u}_k \\
& + \partial_1 \partial_2 r(u_{k+1}, u_{k}, m, p_{k+1}) \hat{u}_{k+1} \tilde{u}_{k} \\
& 
+ \partial_1 \partial_m r(u_n, u_{k-1}, m, p_n) \hat{m} \tilde{u}_n\\
& + \partial_2 \partial_m r(u_{n+1}, u_{n}, m, p_{n+1}) \hat{m} \tilde{u}_n
\end{align}
$$
...
[W_{um}\hat{m}]_k
= 
\partial_1 \partial_m r(u_k, u_{k-1}, m, p_k) \hat{m} \tilde{u}_k
+ 
\partial_2 \partial_m r(u_{k+1}, u_{k}, m, p_{k+1}) \hat{m} \tilde{u}_k.
$$
...
= 
\partial_1 \partial_m r(u_k, u_{k-1}, m, p_k) \tilde{m} \hat{u}_k
+ 
\partial_2 \partial_m r(u_{k+1}, u_{k}, m, p_{k+1}) \tilde{m} \hat{u}_k.
$$
...
[W_{uu}\hat{u}_{k}]_{k}
= 
\partial_1 \partial_1 r(u_k, u_{k-1}, m, p_k) \hat{u}_k \tilde{u}_{k}
+ 
...
[W_{uu}\hat{u}_{k}]_{k+1}
= 
\partial_1 \partial_2 r(u_{k+1}, u_{k}, m, p_{k+1}) \hat{u}_k \tilde{u}_{k+1},
$$


#### Simplifications for implicit Euler

As with the adjoint equation, simplifications can be made when using implicit Euler. For any form with $\partial_2 r(u_{k+1}, u_{k}, m, p_{k+1})$, one can again plug in any $u_{k+1}, u_{k}$ due to linearity in the second argument (and $u_k$ never has mixed nonlinearities with $u_{k+1}$ or $m$). This often saves the need to retrieve $u_{k+1}$ in addition to $u_{k}, u_{k-1}$ within a time step. Again, the current `hippylib` implementation makes these simplifications.