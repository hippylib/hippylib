...
def flatten_parameters(parameters, sep="_"):
    ...
    for keys, value in flatten(parameters):
        ...
        if option in new:
            warn(f"Ignoring duplicate option: {option} (existing value {new[option]}, new value {value})")
        new[option] = value
...
class OptionsManager(object):
    ...
    def __init__(self, parameters, options_prefix):
        ...
        if options_prefix is None:
            self.options_prefix = f"hippylib_{next(self.count)}_"
    ...
    def set_from_options(self, petsc_obj):
        ...
        setFromOptions should only ever be called once.
        This function ensures we do so.
        """
