...
class TestNumpy2Expression(unittest.TestCase):
    ...
    def test1Dv(self):
        ...
        n_np = 2*n
        h = 1./float(n_np)
        f_np = np.zeros( (n_np+1, ncomp) )
        f_np[:,0] = np.linspace(0., 1., n_np+1)
        f_np[:,1] = np.linspace(0., 1., n_np+1)**2
        f_exp = NumpyVectorExpression1D(ncomp)
        f_exp.setData(f_np, h)
        
        f_exp2 = dl.Expression(("x[0]", "x[0]*x[0]"), degree=2)
    ...
    def test2Dv(self):
        ...
        xx, yy = np.meshgrid(x_np, y_np, indexing='ij')
        f_np = np.zeros( (xx.shape[0], xx.shape[1], ncomp))
        f_np[:,:, 0] = xx + 2.*yy
        f_np[:,:, 1] = xx*xx + 2.*yy*yy
        f_exp = NumpyVectorExpression2D(ncomp)
        f_exp.setData(f_np, hx, hy)
        
        f_exp2 = dl.Expression( ("x[0] + 2.*x[1]", 
                                 "x[0]*x[0] + 2.*x[1]*x[1]"), degree=2)
